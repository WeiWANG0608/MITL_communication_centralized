...
from heavy_path_generation_decentralized import get_update_path_decentralized
...
def collect_deliver(world, map_obs_work, num_round_exchange, num_agent_heavy, work_region, states_work_region,
    ...
    region_row = work_region[0][1]-work_region[0][0]+1
    region_col = work_region[0][3] - work_region[0][2] + 1
...
<<<<<<< HEAD:MITL_communication_centralized/env/request_exchange.py
            if round(len(world.agents_heavy[i].path) / world.agents_light.velocity) > 1000:
                no_path = True
                print("Cannot find heavy agents")
                break
            else:
                ...
                for j in range(len(path_sweep[i])):
                    ...
                    if (math.sqrt(abs(path_sweep[i][j][0] - world.agents_heavy[i].p_pos[0]) ** 2 +
                        ...
                        if world.agents_light.num_round > 0 and len(world.agents_light.request_list[i]) > 0:
                            ...
                            new_path, new_spec, state_dict_new = get_update_path_decentralized(current_pos, current_time,
                            ...
                            if len(new_path) > 1000:
                                print(f"{bcolors.WARNING} Something wrong in the obtained trajectory")
                                no_path = True
...
>>>>>>> 91c387d511a21f74a672e7068e51ca73cab297b3:MAS_search_rescue/env/request_exchange.py
                              math.floor(len(world.agents_heavy[i].path) / world.agents_light.velocity),
                              ". Pos_heavy: ", [world.agents_heavy[i].p_pos[0], world.agents_heavy[i].p_pos[1]],
                              ". Pos_light: ", [path_sweep[i][j][0], path_sweep[i][j][1]],
                              ". Distance is: ", abs(world.agents_heavy[i].p_pos[0] - path_sweep[i][j][0]) +
...
>>>>>>> 91c387d511a21f74a672e7068e51ca73cab297b3:MAS_search_rescue/env/request_exchange.py
    ...
    print("request_exchange. there is not a path", no_path)
    return no_path, path_detect_round, independent_path_2d, record_working_region, record_task_left, deliver_time
