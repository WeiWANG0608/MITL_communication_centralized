...
import numpy as np
from heavy_WTS_decentralized import TransitionSystem_decentralized, total_states_decentralized
from heavy_MITL2TA_decentralized import MITL2TBA_decentralized
import subprocess
import json
import copy
import os
...
class heavy_path_decentralized:
    ...
    def __init__(self, map_ori, specifications, filefolder, flag, region_row, region_col, init_pos=0):
        self.map = map_ori
    ...
    def get_product(self):
        ...
        wts_heavy = TransitionSystem_decentralized(self.map, doc_declaration, wts_declaration, self.init_pos)
        ...
        ta_heavy = MITL2TBA_decentralized(state_wts, locations_wts, labels_wts, edges_wts, self.spec_list,
                                          doc_declaration, ta_declaration, 0)
        ...
        if self.flag in ['R0', 'R1', 'R2', 'R3', 'R4']:
            f = open(self.filefolder + "/output/" + 'Time_de_' + str(self.region_row) + "*" + str(self.region_col)
                     + ".txt", "a")  # append mode
...
def get_work_map_decentralized(map_ori, map_list, work_region):
...
def get_path_cell2coord_decentralized(path, states_dict):
    path_coord = []
...
def if_collab_meet_promise_decentralized(spec):
    ifcollab = False
    ifmeet = False
    ifpromise = False
    if len(spec) > 4:
...
def get_promise_request_decentralized(spec, index, map_local, filefolder, flag, region_row, region_col, init_p):
    flag = "Re"+flag
    veri_spec = copy.deepcopy(spec)
    promise = []
    request = copy.deepcopy(veri_spec[index])
    request[index][1] = request[index][2]
    if veri_spec[index][1] < veri_spec[index][2]:
        for i in range(1, int(veri_spec[index][2]) - int(veri_spec[index][1])):
            ...
            path_heavy_promise = heavy_path_decentralized(map_local, veri_spec, filefolder, flag, region_row,
                                                          region_col, init_p).get_path()
...
def get_cooperation_map_decentralized(map_ori, map_list, coop_region, target_h):
...
def get_update_path_decentralized(current_pos, current_time, coop_region, request, recipient, task_left,
                  state_dict_local, map_ori, map_list, filefolder, region_row, region_col):
    """
    ...
    """
    coop_path = []
    map_coop = get_cooperation_map_decentralized(map_ori, map_list, coop_region, recipient)
    state_dict_new = total_states_decentralized(map_coop)
    ...
    for i in range(len(spec_new)):
        if spec_new[i][2] != '0':
<<<<<<< HEAD:MITL_communication_centralized/env/heavy_path_generation_decentralized.py
            spec_backup.append(spec_new[i])
=======
            spec_backup.append( spec_new[i])
>>>>>>> 91c387d511a21f74a672e7068e51ca73cab297b3:MAS_search_rescue/env/heavy_path_generation.py
    spec_new = spec_backup
    print(f"{bcolors.ENDC}The new specification after the time update is ", spec_new)
    if len(spec_new) > 0:
        path_heavy = heavy_path_decentralized(map_coop, spec_new, filefolder, flag, region_row, region_col,
                                              current_pos).get_path()
    else:
        path_heavy = []
        print(f"{bcolors.FAIL}Tried, but cannot achieve (find) cooperation")

    if len(path_heavy) > 0:
        print(f"{bcolors.OKBLUE}The cooperation can be achieved! ")
        coop_path = get_path_cell2coord_decentralized(path_heavy, state_dict_new)
...
def get_independent_requests_tasks(specs, map_regions, agents_h, file_folder, region_row, region_col):
    num = len(specs)
    ...
    for i in range(num):
        ...
        for k in range(len(specs[i])):
            ifcollab, ifmeet, ifpromise = if_collab_meet_promise_decentralized(specs[i][k])
            ...
            # meet requests
                ...
                veri_path = heavy_path_decentralized(map_regions[i], veri_spec, file_folder, flag, region_row,
                                                     region_col, agents_h[i]).get_path()
                if len(veri_path) > 0:
                    j = int(specs[i][k][4][1:len(specs[i][k][4])])
                    independent_requests[i][j].append(specs[i][k])
                    ...
                    meet_promise, meet_request = get_promise_request_decentralized(veri_spec, index, map_regions[i], file_folder,
                                                                     flag, agents_h[i])
