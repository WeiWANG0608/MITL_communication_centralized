...
class specification_decentralized(object):
    def __init__(self):
        self.operator = ''
        self.start_time = 0
        self.end_time = 0
...
class MITL2TBA_decentralized:
    ...
    def eventually(self, spec):
        ...
        for i in range(len(self.edges_ta)):
            ...
            if int(spec.clock[2:len(spec.clock)]) == int(self.edges_ta[i][0][1:len(self.edges_ta[i][0])]) and \
                    self.edges_ta[i][0] != self.edges_ta[i][1]:
    ...
    def split_specification(self):
        ...
        """
        speclist = [specification_decentralized() for i in range(len(self.ori_spec))]
        for i, spec in enumerate(speclist):
            spec.clock = 'cl' + str(i + 1)  # the index of the clock for agent i
            addDeclaration(self.ta_decl, ['int', 'cl' + str(i + 1)])
            spec.cycle = 'cy' + str(i + 1)
            addDeclaration(self.ta_decl, ['int', 'cy' + str(i + 1)])  # the number of GF happens
            spec.operator = self.ori_spec[i][0]  # temporal operator
            spec.start_time = self.ori_spec[i][1]
            spec.end_time = self.ori_spec[i][2]
            spec.task = self.ori_spec[i][3]  # target cell
            spec.sync = ['p' + str(i + 1), 'np' + str(i + 1)]  # the index of commands
            addDeclaration(self.doc_decl, ['chan', 'p' + str(i + 1)])
            addDeclaration(self.doc_decl, ['chan', 'np' + str(i + 1)])
            spec.update = 'veri' + str(i + 1)  # the index of verification character
